...
def test_deepseek_v4_annotation_requires_model_type():
    ...
    #
    # Upstream expresses "not DeepseekV4" purely as hf_config.model_type, because
    # it dropped the packed group_and_unify path. This fork keeps that path, and
    # it proves the model family from the merged MLA spec's model_version (set
    # only by models/deepseek_v4/attention.py) rather than from the config, so the
    # negative case has to withhold that marker too. See _annotate_eagle_groups.
    groups = get_kv_cache_groups(
