...
### 171. `c866ba9d11` -> `894eb2c1b` — [KV Connector] Support heterogeneous TP sharing in Mooncake Store Connector

*Minimum gate (hybrid).* Clean apply, no conflict, but `store/worker.py` is fork-diverged, so full depth rather than the minimum gate. What landed: upstream reworks the Mooncake store connector so cache groups can be shared across heterogeneous TP — `LBHNCStoreLayout` plus per-group `_spec_tp_replication_factor`/`_compute_group_tp_replication_factors` (DCP and Mamba → 1, MLA/SlidingWindowMLA → tp_size, else tp_size//num_kv_head), `_build_token_databases` giving each group its own tp_rank/key namespace, rewritten save/lookup paths, new `test_mooncake_store_layout.py`, deleted `test_mooncake_store_prepare_values.py`.

Fork delta in this file is 40 lines: the `group_participates` mechanism (skip groups whose `kv_cache_spec` does not participate in prefix caching — two thread constructors, `_sub_block_tail_puts`, both `_handle_request`s, `lookup`) and the fork's `use_eagle()` substitution for upstream's `use_eagle_block_drop()`, a method this tree does not define (`vllm/config/speculative.py:1865` defines only `use_eagle()`), so upstream-parent's `callable(getattr(...))` guard made it silently false here. All intact after the pick: gap invariance holds with zero non-header body lines in the delta diff (only hunk headers shifted), 40/40 fork lines present verbatim, silent-loss symbol scan empty, ast.parse OK, and the guards are still in their original enclosing functions (ast-located) with `use_eagle=use_eagle` still at the `MooncakeStoreCoordinator(` call — the verbatim check alone was insufficient there, because a moved line would still have "passed", so the call site was read.

Coherence is the part that could have broken silently, since upstream rewrote the loops the fork guards. It holds: `_build_token_databases` appends exactly one db per KV-cache group with no skip, so `token_dbs[g_idx]` stays aligned with `_kv_cache_groups[g_idx]`; every per-group container (`load_mask_per_group`, `lookup_masks`, `_lookup_key_prefixes`, `_group_tp_replication_factors`, `block_ids_per_group`) is indexed by `g_idx`, and the new flat key lists carry `group_indices`/`candidate_meta` per entry rather than positional indices, so a fork `continue` can only omit a group's keys, never desynchronise them. `_spec_tp_replication_factor` recognises the fork's `SlidingWindowMLASpec`, returns 1 for any `MambaSpec` (the production hybrid), and falls through to the GQA formula instead of raising on an unknown spec. `use_eagle_block_drop` now appears nowhere in vllm/ or tests/.
